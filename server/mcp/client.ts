...
function skillEnvFromLumi(lumi: any): Record<string, string> | undefined {
  const env: Record<string, string> = {};
  if (lumi.requiresApiKey && lumi.apiKeyEnv) {
    env[lumi.apiKeyEnv] = `\${${lumi.apiKeyEnv}}`;
  }
  if (Array.isArray(lumi.envKeys)) {
    for (const key of lumi.envKeys) {
      if (typeof key === 'string' && key.trim()) env[key] = `\${${key}}`;
    }
  }
  return Object.keys(env).length > 0 ? env : undefined;
}

function trimProcessOutput(output: string, maxLength = 2000): string {
  const normalized = output.replace(/\r/g, '').trim();
  if (normalized.length <= maxLength) return normalized;
  return `...${normalized.slice(-maxLength)}`;
}

function formatServerCommand(config: MCPServerConfig): string {
  return [config.command || '', ...(config.args || [])].filter(Boolean).join(' ');
}

function withStdioDiagnostics(
  name: string,
  config: MCPServerConfig,
  err: any,
  stderr: string,
  ...
  const details = trimProcessOutput(stderr);
  if (!details) return new Error(original);

  const command = formatServerCommand(config);
  return new Error(
    `MCP server "${name}" failed to start. ${original}. Command: ${command}. Stderr: ${details}`,
  );
}
...
class MCPClientManager {
  ...
  async installSkill(name: string, sourceDir: string, allowUpgrade = false): Promise<string> {
    ...
    const sourceHasIndex = fs.existsSync(path.join(sourceDir, 'index.ts'));
    const runCommandOnly = !!sourcePkg.lumi?.runCommand && !sourceHasIndex;
    ...
    if (runCommandOnly) {
      console.log(`[MCP] Skipping dependency install for external runtime skill "${name}"`);
    } else {
      await this.installDepsSync(destDir);
    }
    this.patchInstalledMetadata(destDir);
    // Also stamp the installed version from source
    this.patchInstalledVersion(destDir, sourcePkg.version || '0.0.0');
...
`;
  ...
  private registerLocalSkill(name: string, skillDir: string, pkg: any): void {
    ...
    const needsManualEnable = !!(lumi.requiresApiKey || lumi.requiresSetup || isExternalRuntime);
    const commonMetadata = {
      ...
      requiresApiKey: !!lumi.requiresApiKey,
    ...
    if (lumi.runCommand) {
      config[name] = {
        ...
        args: lumi.runArgs || [],
        env: skillEnvFromLumi(lumi),
        cwd: skillDir,
        enabled: !needsManualEnable,
        ...commonMetadata,
      };
    } else {
  ...
  private async connectServer(name: string, config: MCPServerConfig): Promise<MCPToolDef[]> {
    ...
    if (transportType === 'ws' && config.url) {
      ...
      transport = new StdioClientTransport({
        ...
        cwd: config.cwd ? expandPortablePath(config.cwd) : undefined,
        stderr: 'pipe',
      });
      transport.stderr?.on('data', (chunk: Buffer | string) => {
        stdioStderr += chunk.toString();
      });
    } else {
    ...
    try {
      ...
      if (transportType === 'stdio') {
        throw withStdioDiagnostics(name, config, err, stdioStderr);
      }
      throw err;
    }
