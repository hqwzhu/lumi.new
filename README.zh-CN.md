...
[https://github.com/hqwzhu/lumi.new/releases/tag/windows-v3.0.5](https://github.com/hqwzhu/lumi.new/releases/tag/windows-v3.0.5)

普通用户只需要下载这个文件：

[`Lumi OS 安装.exe`](https://github.com/hqwzhu/lumi.new/releases/download/windows-v3.0.5/Lumi%20OS%20%E5%AE%89%E8%A3%85.exe)
...
| 文件 | 用途 |
| --- | --- |
| `Lumi OS 安装.exe` | Windows 安装器，普通用户下载这个 |
| `Lumi OS 卸载.cmd` | Windows 卸载工具，需要卸载时运行这个 |
| `LumiOS-Windows-3.0.5.zip` | 完整发布包，适合备份或内部交付 |
...
需要卸载时，运行发布包里的 `Lumi OS 卸载.cmd`，或在 Windows 设置中卸载 Lumi OS。
...
Windows 版本启用一机一授权。用户第一次启动 Lumi OS 时会先看到机器码；运营人员在授权码生成器中输入机器码，生成授权码后发给用户。用户粘贴授权码并点击激活后，才会进入 API Key 配置向导。
