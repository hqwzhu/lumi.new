...
`;
  ...
  try {
    ...
    // Check for duplicate before overwriting
    if (fs.existsSync(skillDir)) {
      const existingPkg = path.join(skillDir, 'package.json');
      if (fs.existsSync(existingPkg)) {
        try {
          const pkg = JSON.parse(fs.readFileSync(existingPkg, 'utf-8'));
          if (pkg.lumi?.autoGenerated) {
            return { success: false, error: `Skill "${skillName}" already exists — remove it first before regenerating.`, directory: skillDir };
          }
        } catch {}
      }
    }
